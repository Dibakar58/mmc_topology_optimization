function MMC3D_1(DL,DW,DH,nelx,nely,nelz,xInt,yInt,zInt,vInt,volfrac)
% DL=64 ;
% DW=8 ;
% DH=32 ;
% 
...
fixNd =[(kN-1)*nNodfc+(jN-1)*(nelx+ 1)+ 1]; %fixed   nodes
fixDof= [3*fixNd(:)-2; 3*fixNd(:) ; 3*fixNd(:)] ;	                          %fixed dofs
[jE, kE] = meshgrid(1:nely, 1:nelz);     
fixEle = [(kE-1)*nelx*nely+(jE-1)*nelx+1]; %elements related to fixed nodes
freeDof = setdiff([1:nDof],fixDof) ;
                        % index of free dofs 
loadNd = (nely/2+ 1)*(nelx+1);           
loadDof = 3*loadNd;	                          % loaded nodes and loaded dofs
loadEle = nelx*(nely/2+[0 1]);	                         % elements related to loaded nodes
F = fsparse(loadDof' ,1, -1/length(loadNd), [nDof, 1]);	 %  external load vector
